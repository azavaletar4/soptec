    ...
    <div class="table-shell mb-6">
      <table class="w-full text-sm min-w-[720px]">
        ...
        <tbody>
          ...
          <tr v-for="c in filteredClientsList" :key="c.id" class="border-t border-slate-800 hover:bg-slate-900/50">
            <td class="px-4 py-3">
            ...
            <td class="px-4 py-3 text-right space-x-3 whitespace-nowrap">
              ...
              <button v-if="canManageClients" class="text-red-500/80 hover:text-red-400 text-xs" @click="handleDelete(c)">Eliminar</button>
            </td>
          </tr>
        </tbody>
      </table>
    ...
    <div class="rounded-xl border border-slate-800 bg-slate-900 p-5 mb-6">
      <div class="flex flex-wrap items-center justify-between gap-3 mb-3">
        ...
        <select
          ...
          class="px-3 py-2 rounded-lg border border-slate-700 bg-slate-950 text-sm"
        >
      ...
      <p v-else-if="!unlinkedSecrets.length" class="text-sm text-green-400">Todos los usuarios PPPoE de este router ya tienen contrato.</p>
      <div v-else class="rounded-lg border border-slate-800 overflow-hidden overflow-x-auto">
        <table class="w-full text-sm min-w-[560px]">
          ...
          <tbody>
            <tr v-for="s in unlinkedSecrets" :key="s['.id']" class="border-t border-slate-800">
              <td class="px-4 py-2 font-mono text-xs">{{ s.name }}</td>
              <td class="px-4 py-2 text-slate-400">{{ s.profile }}</td>
              <td class="px-4 py-2 text-slate-400">{{ s.comment || '—' }}</td>
              <td class="px-4 py-2 text-right">
                <button v-if="canManageClients" class="text-sky-400 hover:underline text-xs" @click="openCreate(s)">Crear cliente</button>
    ...
    <Teleport to="body">
      <div v-if="showModal" class="modal-overlay">
        ...
        >
          ...
          <p v-if="pendingPppoeHint" class="text-xs text-sky-400/80 mb-3">
          ...
          <div class="grid grid-cols-2 gap-3 mb-4">
            <div>
              <div class="flex items-center justify-between mb-1">
                <label class="block text-xs text-slate-400">Zona</label>
                <button type="button" class="text-xs text-sky-400 hover:text-sky-300" @click="showNewZone = !showNewZone">
              ...
              <div v-else class="flex gap-2">
                <input
                  ...
                  class="flex-1 min-w-0 px-3 py-2 rounded-lg border border-slate-700 bg-slate-950 text-sm"
